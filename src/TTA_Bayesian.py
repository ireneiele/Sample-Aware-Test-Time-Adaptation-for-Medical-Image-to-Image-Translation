...
def opt_ANet(adaptors, opt, task_model, rec_models=None, stable=False, model_type='AENet', AE=None, return_layers=None):
    ...
    # thr = 0.0012 #task traslazione T1 T2 su Brats
    # thr_online = thr # task traslazione T1 T2 su IXI
    ...
    for j, batch in tqdm(enumerate(dataset.dataloader)):  # arriva il campione di test
        ...
        # printo rec_loss e il nome dell'immagine
        print(f'img name: {img_path_task_model}', f'loss task model: {rec_loss}', f'PSNR: {psnr_score_task_model}')

        # if rec_loss > 0.0063: #task denoising
        # if rec_loss > 0.0012:  # task traslazione T1 T2
        # if rec_loss > 0.012:  # task traslazione T1 T2
        # if rec_loss > 0.0017: # IXI 20 epoche
        if rec_loss > thr:
        #if rec_loss > 0: # serve per provare approccio competitor
            count_tta += 1
            start_time = time.time()
            # Avvia l'ottimizzazione bayesiana
            def objective(trial):
                n = 7
                # 1. Configurazione binaria (es. 7 bit)
                config_bits = [trial.suggest_int(f"b{i}", 0, 1) for i in range(n)]  # es: [1, 0, 1, 1, 0, 0, 1]
                # Escludi la configurazione con tutti zeri
                if sum(config_bits) == 0:
                    return float('inf')  # oppure: raise optuna.exceptions.TrialPruned()

                # 2. Traduci in indici di layer selezionati
                comb_to_print = [i for i, val in enumerate(config_bits) if val == 1]

                chosen_comb = [0] + [x + 1 for x in comb_to_print] + [n + 1]
                ...
                return min(loss_output)
            
            study = optuna.create_study(direction="minimize", sampler=optuna.samplers.TPESampler(n_startup_trials=5))
            study.optimize(objective, n_trials=20)  # puoi regolare n_trials

            # Ottieni la miglior combinazione trovata
            best_config = [i for i in range(n) if study.best_trial.params[f"b{i}"] == 1]
            used_comb = '_'.join([str(i + 1) for i in best_config])  # es: "1_2_5"

            # Costruisci opt.return_layers corretti per uso finale
            chosen_comb = [0] + [x + 1 for x in best_config] + [n + 1]
            chosen_comb = [return_layers[i] for i in chosen_comb]
            opt.return_layers = chosen_comb
            compute_tnet_dim(opt)
            ...
            row = {'img_name': img_path, 'config': used_comb, 'MAE': mae_score, 'PSNR': psnr_score,
                'SSIM': ssim_score, }
            ...
            # se j % 200 == 0:  # salviamo anche loss_output
            # if stop_plot < 50:
            if batch['A_name'][0] in ['C227_681-069','00423', 'IXI049-HH-1358-T1_slice_11']:
            # if batch['A_name'][0] in ['IXI049-HH-1358-T1_slice_11']:
            ...
            row = {'img_name': img_path_task_model, 'config': None, 'MAE': mae_score, 'PSNR': psnr_score,
                'SSIM': ssim_score}
            df_new = pd.DataFrame([row])
            df_o = pd.concat([df_o, df_new], ignore_index=True)
...
if __name__ == '__main__':
    ...
    # task IXI
    # load_path = '/mimer/NOBACKUP/groups/snic2022-5-277/cadornato/pytorch-CycleGAN-and-pix2pix/checkpoints/cyclegan_IXI/50_net_G_A.pth'

...
def backward_step(opt, all_comb, batch, model_type='AENet'):
    ...
    #candidate_layers = return_layers[1:-1]
    loss_config_output = pd.DataFrame(columns=['config', 'loss_output', 'loss_tot', 'PSNR'])
    for i in range(len(return_layers[1:-1])):
        # se l'elemento è già presente in all_comb, non lo considero
        if i in all_comb:
            continue
        comb_to_print = all_comb + [i] # prendi i primi 'i' elementi di all_comb
        chosen_comb = list(comb_to_print)  # prendi una configurazione
        ...
        adaptors.reset()  # resetta per ogni campione i pesi
